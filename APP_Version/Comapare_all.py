...
if extension != ".png":
    ...
    #save image
    cv2.imwrite('Comparison.png',img_binary)
...
# Take full DB
#DIR = 'SQL_DB/'
#sumgrammes=len([name for name in os.listdir(DIR) if os.path.isfile(os.path.join(DIR, name))])

#grammes = sumgrammes # rows of DB -1?
grammes=0
try:
  ...
  sql_select_query = """select * from signat"""
...
image_from_User = "Comparison.png"
...
for i in range(1,6):
  if i < 5:
    f.write(str(maxx[grammes-i][0]) +"|"+ maxx[grammes-i][1] +"|"+ str(maxx[grammes-i][2])+"\n")    # creating results
  else:
    f.write(str(maxx[grammes-i][0]) +"|"+ maxx[grammes-i][1] +"|"+ str(maxx[grammes-i][2]))
f.close()
